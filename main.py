...
def main():
    ...
    parser.add_argument('--cls_w', type=float, default = 1, help='multiplicative factor for target adv. loss')


    opt = parser.parse_args()
    print('cls_w', opt.cls_w)
    print(opt)
    ...
    source_trainloader = torch.utils.data.DataLoader(source_train, batch_size=opt.batchSize, shuffle=True,  drop_last=True)
    source_valloader = torch.utils.data.DataLoader(source_val, batch_size=opt.batchSize, shuffle=False, drop_last=False)
    targetloader = torch.utils.data.DataLoader(target_train, batch_size=opt.batchSize, shuffle=True,  drop_last=True)
